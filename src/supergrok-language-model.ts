...
import {
  supergrokProviderOptionsSchema,
  type SupergrokProviderOptions,
} from './supergrok-provider-options'
...
function convertUsage(
  usage: z.infer<typeof xaiUsageSchema> | null | undefined,
...
export class SupergrokLanguageModel implements LanguageModelV3 {
    ...
    if (supergrokOptions?.searchParameters != null) {
      ...
      const sources = sp.sources?.map(source => {
        ...
        switch (source.type) {
          ...
          case 'rss':
            return { ...base, links: source.links }
        }
      })
  ...
  async doGenerate(
    ...
    const result = (await postJsonToApi({
      ...
      value: z.infer<typeof xaiChatResponseSchema>
      rawValue?: unknown
