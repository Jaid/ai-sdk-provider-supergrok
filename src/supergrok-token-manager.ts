...
export interface SupergrokTokenManagerOptions {
  ...
  now?: () => number
  onTokenRefresh?: (tokens: SupergrokTokenPair) => void | Promise<void>
}
...
export class SupergrokTokenManager {
  ...
  readonly now: () => number
  readonly onTokenRefresh: ((tokens: SupergrokTokenPair) => void | Promise<void>) | undefined
  ...
  get accessToken(): string | undefined { return this.#accessToken }
  get refreshToken(): string | undefined { return this.#refreshToken }
  get accessTokenExpiresAt(): number | undefined { return this.#accessTokenExpiresAt }

  /** Whether a refresh token is available for rotational refresh. */
  hasRefreshToken(): boolean { return Boolean(this.#refreshToken) }

  /** True if the access token JWT has expired (or will expire within `skewMs`). */
  accessTokenIsExpired(skewMs = 0): boolean {
    if (this.#accessTokenExpiresAt == null) return false
  ...
  async getAccessToken(): Promise<string> {
    ...
    if (!this.#accessToken) {
      throw new Error(
        'SuperGrok authentication requires either accessToken or refreshToken.',
      )
    }

    if (this.accessTokenIsExpired()) {
      throw new Error(
        'The SuperGrok access token is expired and no refresh token was provided.',
      )
    }
  ...
  async #doRefresh(): Promise<SupergrokTokenPair> {
    ...
    try {
      if (responseText) data = JSON.parse(responseText) as TokenRefreshResponse
    } catch {
    ...
    if (!response.ok) {
      throw new Error(
        `xAI token refresh failed (${response.status})${responseText ? `: ${responseText}` : ''}`,
      )
    }

    if (!data?.access_token || typeof data.access_token !== 'string') {
      throw new Error(
        `xAI token refresh response missing access_token: ${responseText}`,
      )
    }
    ...
    this.#accessTokenExpiresAt =
      typeof data.expires_in === 'number' && data.expires_in > 0
        ? this.now() + data.expires_in * 1000
        : getJwtExpiresAt(data.access_token)
