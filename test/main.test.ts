...
const createJwt = (expiresAt: number) =>
  `header.${encodeBase64Url({ exp: Math.floor(expiresAt / 1000) })}.signature`
...
describe('mapSupergrokFinishReason', () => {
  it('maps stop', () => {
    expect(mapSupergrokFinishReason('stop')).toEqual({ unified: 'stop', raw: 'stop' })
  })

  it('maps length', () => {
    expect(mapSupergrokFinishReason('length')).toEqual({ unified: 'length', raw: 'length' })
  })

  it('maps content_filter', () => {
    expect(mapSupergrokFinishReason('content_filter')).toEqual({ unified: 'content-filter', raw: 'content_filter' })
  })

  it('maps tool_calls', () => {
    expect(mapSupergrokFinishReason('tool_calls')).toEqual({ unified: 'tool-calls', raw: 'tool_calls' })
  })

  it('handles null/undefined', () => {
    expect(mapSupergrokFinishReason(null)).toEqual({ unified: 'other', raw: undefined })
    expect(mapSupergrokFinishReason(undefined)).toEqual({ unified: 'other', raw: undefined })
  })

  it('maps unknown to other', () => {
    expect(mapSupergrokFinishReason('unknown_reason')).toEqual({ unified: 'other', raw: 'unknown_reason' })
  })
})
...
describe('convertToXaiMessages', () => {
  it('converts system message', () => {
    ...
    expect(messages).toEqual([{ role: 'system', content: 'Be helpful.' }])
  })
  ...
  it('converts tool result message', () => {
    ...
    expect(messages).toEqual([{ role: 'tool', tool_call_id: 'call_1', content: 'Sunny' }])
  })
...
describe('prepareTools', () => {
  it('returns undefined for empty tools', () => {
    const result = prepareTools({ tools: undefined, toolChoice: undefined })
  ...
  it('converts function tools', () => {
    const result = prepareTools({
      tools: [{ type: 'function', name: 'get_weather', description: 'Get weather', inputSchema: { type: 'object' } }],
  ...
  it('maps tool choice modes', () => {
    expect(prepareTools({ tools: [{ type: 'function', name: 'f', description: '', inputSchema: {} }], toolChoice: { type: 'auto' } }).toolChoice).toBe('auto')
    expect(prepareTools({ tools: [{ type: 'function', name: 'f', description: '', inputSchema: {} }], toolChoice: { type: 'none' } }).toolChoice).toBe('none')
    expect(prepareTools({ tools: [{ type: 'function', name: 'f', description: '', inputSchema: {} }], toolChoice: { type: 'required' } }).toolChoice).toBe('required')
    expect(prepareTools({ tools: [{ type: 'function', name: 'f', description: '', inputSchema: {} }], toolChoice: { type: 'tool', toolName: 'f' } }).toolChoice).toEqual({ type: 'function', function: { name: 'f' } })
  })
})
...
describe('SupergrokTokenManager', () => {
  ...
  it('refreshes when only refresh token is provided', async () => {
    ...
    const tm = new SupergrokTokenManager({
      ...
      fetch: async () =>
        new Response(
          JSON.stringify({ access_token: refreshedAccess, expires_in: 3600 }),
          { headers: { 'content-type': 'application/json' } },
        ),
    })
  ...
  it('single-flights concurrent refresh calls', async () => {
    ...
    const tm = new SupergrokTokenManager({
      ...
      fetch: async () => {
        refreshCount++
        return new Response(
          JSON.stringify({ access_token: createJwt(Date.now() + 600_000), expires_in: 3600 }),
          { headers: { 'content-type': 'application/json' } },
        )
      },
    })
  ...
  it('calls onTokenRefresh callback', async () => {
    ...
    const tm = new SupergrokTokenManager({
      ...
      fetch: async () =>
        new Response(
          JSON.stringify({ access_token: createJwt(Date.now() + 600_000), refresh_token: 'rotated', expires_in: 3600 }),
          { headers: { 'content-type': 'application/json' } },
        ),
    })
...
test('integrates with mocked API', async () => {
  ...
  const response = await generateText({
    ...
    messages: [{ role: 'user', content: 'Which animal is dangerous?' }],
  })
